...
const AdminPanel = ({ fetchFruits, fruits, cart, setCart }) => {
  ...
  const addFruit = async () => {
    try {
      await axios.post('http://localhost:5000/addFruit', fruitData);
  ...
  const deleteFruit = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/deleteFruit/${id}`);
      fetchFruits();
    } catch (error) {
  ...
  const editExistingFruit = async () => {
    try {
      await axios.put(`http://localhost:5000/updateFruit/${editFruit._id}`, fruitData);
      setEditFruit(null);
      setFruitData({ name: '', image: '', quantity: 0, price: 0 });
      fetchFruits();
    } catch (error) {
...
const App = () => {
  ...
  const fetchFruits = async () => {
    try {
      const response = await axios.get('http://localhost:5000/getFruits');
      ...
      for (const cartItem of cart) {
        ...
        if (originalFruit) {
          ...
          const response = await axios.put(
            `http://localhost:5000/updateFruit/${cartItem._id}`,
  ...
  <svg viewBox='0 0 200 200' width='200' height='200' xmlns='http://www.w3.org/2000/svg' className="link__svg" aria-labelledby="link2-title link2-desc">
    ...
    <text className="link__text">
      <textPath href="#link-circle-alt" stroke="none">
        ...
        <TabPanel>
  <ShoppingCart cart={cart} goBackToChooseFruits={() => setActiveTab(2)} />
  {cart.length > 0 && (
    <button className="fixed-button-payment" onClick={() => {
      dispenseCart();
      goToDispenseTab();
